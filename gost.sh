...
function Install_ct() {
  ...
  if [[ ${addyn} == [Yy] ]]; then
    ...
    mv gost /usr/bin/gost
    chmod -R 777 /usr/bin/gost
    wget --no-check-certificate https://ws.bxcc.de/https://raw.githubusercontent.com/sjhxlll/Multi-EasyGost/master/gost.service && chmod -R 777 gost.service && mv gost.service /usr/lib/systemd/system
    mkdir /etc/gost && wget --no-check-certificate https://jiashu.1win.eu.org/https://raw.githubusercontent.com/sjhxlll/Multi-EasyGost/master/config.json && mv config.json /etc/gost && chmod -R 777 /etc/gost
  fi
