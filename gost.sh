...
function Install_ct() {
  ...
  if [[ ${addyn} == [Yy] ]]; then
    ...
    rm -rf gost-linux-"$bit"-"$ct_new_ver".gz
    wget --no-check-certificate https://jiashu.1win.eu.org/https://github.com/ginuerzh/gost/releases/download/v"$ct_new_ver"/gost-linux-"$bit"-"$ct_new_ver".gz
    gunzip gost-linux-"$bit"-"$ct_new_ver".gz
    mv gost-linux-"$bit"-"$ct_new_ver" gost
    mv gost /usr/bin/gost
    chmod -R 777 /usr/bin/gost
    wget --no-check-certificate https://jiashu.1win.eu.org/https://raw.githubusercontent.com/sjhxlll/Multi-EasyGost/test/gost.service && chmod -R 777 gost.service && mv gost.service /usr/lib/systemd/system
    mkdir /etc/gost && wget --no-check-certificate https://jiashu.1win.eu.org/https://raw.githubusercontent.com/sjhxlll/Multi-EasyGost/test/config.json && mv config.json /etc/gost && chmod -R 777 /etc/gost
  fi
